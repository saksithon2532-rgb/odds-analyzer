...
st.set_page_config(page_title="Odds Analyzer Pro V10.3 (Dark Edition)", page_icon="⚽", layout="centered")

# ปรับแต่ง CSS ให้เป็น Dark Mode พื้นหลังสีดำ สบายตาเวลาดูบอลดึก
st.markdown("""
    <style>
    /* พื้นหลังหลักและแถบด้านบน */
    .stApp {
        background-color: #0e1117;
        color: #e6edf3;
    }
    /* สีฟอนต์หัวข้อและข้อความทั่วไป */
    h1, h2, h3, h4, h5, h6, p, span, label {
        color: #f0f6fc !important;
    }
    /* กล่องข้อความและช่องกรอก Input */
    ...
    .stButton > button:hover {
        background: linear-gradient(90deg, #388bfd, #2ea043) !important;
    }
    </style>
""", unsafe_allow_html=True)

st.title("⚽ เครื่องมือวิเคราะห์ราคาบอล (Pro V10.3)")
st.caption("ระบบคำนวณ 2 หรือ 3 แถวราคา ครอบคลุมเรต 0 ถึง 7 ลูก พร้อมชี้เป้าตัวที่น่าเล่นที่สุด")

# เรตราคาแฮนดิแคปจัดเต็ม (ตั้งแต่ 0 ถึง 7.0 ลูก)
HDP_OPTIONS = [
...
# เรตราคาสูง-ต่ำจัดเต็ม (ตั้งแต่ 0.5/1 ถึง 7.5 ลูก)
OU_OPTIONS = [
    "0.5-1 (ครึ่งควบลูก)", "1.0 (หนึ่งลูก)", "1-1.5 (ลูกควบลูกครึ่ง)",
    "1.5 (ลูกครึ่ง)", "1.5-2 (ลูกครึ่งควบสอง)", "2.0 (สองลูก)", "2-2.5 (สองควบสองครึ่ง)",
    "2.5 (สองลูกครึ่ง)", "2.5-3 (สองครึ่งควบสาม)", "3.0 (สามลูก)", "3-3.5 (สามควบสามครึ่ง)",
    "3.5 (สามลูกครึ่ง)", "3.5-4 (สามครึ่งควบสี่)", "4.0 (สี่ลูก)", "4-4.5 (สี่ควบสี่ครึ่ง)",
    "4.5 (สี่ลูกครึ่ง)", "4.5-5 (สี่ครึ่งควบห้า)", "5.0 (ห้าลูก)", "5-5.5 (ห้าควบห้าครึ่ง)",
    "5.5 (ห้าลูกครึ่ง)", "5.5-6 (ห้าครึ่งควบหก)", "6.0 (หกลูก)", "6-6.5 (หกควบหกครึ่ง)",
    "6.5 (หกลูกครึ่ง)", "6.5-7 (หกครึ่งควบเจ็ด)", "7.0 (เจ็ดลูก)", "7-7.5 (เจ็ดควบเจ็ดครึ่ง)", "7.5 (เจ็ดลูกครึ่ง)"
]
...
for i in range(num_rows):
    ...
    our = st.selectbox(f"เรต สูง-ต่ำ แถว {i+1}", OU_OPTIONS, index=6 if i==0 else 5, key=f"our_{i}")
